...
Read `NEXT.md` for current work and remaining acceptance gates. C2/C3 code is merged into `main` from `codex/c2-c3-reminders-agents`; 320 Rust tests and desktop/web builds pass. Native synthetic CLI refresh, reminder submission and restart catch-up are verified. C2/C3 production app and matching `dt` CLI were installed on 2026-09-21, preserving all existing task data and passing new backup/restore verification. Live Google/phone acceptance and agent workflow activation remain separate gates; see `docs/c2-c3-verification.md`. C1 backup/restore is implemented and merged into `main`; see `docs/c1-verification.md` and `docs/backup-recovery.md`. Native Settings smoke passed in a standalone synthetic test bundle; the signed release was installed in `/Applications/Nimble.app` on 2026-09-21 and completed its first real local backup. Private archive `marcosevilla/nimble-backups` is connected and its first upload was acknowledged on 2026-09-21.
