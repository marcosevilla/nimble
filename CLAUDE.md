...
- Vercel **Password Protection needs the paid Advanced Deployment Protection add-on** (unavailable on Hobby AND on this Pro team), and Vercel Authentication **cannot cover production deployments** on this plan. Net effect: deployment-specific URLs are protected, an auto-assigned production domain is NOT. After any `vercel deploy --prod`, verify `GET /` returns 302 and not 200.
