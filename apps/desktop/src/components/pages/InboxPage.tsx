      ...
      <PageHeader
        ...
        actions={
          <button
            ...
            disabled={importing}
            className="flex items-center gap-1 rounded-md px-2 py-1 text-meta text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:opacity-50"
...
function InboxTaskRow({
  ...
  return (
    <div className={cn(
      'group flex items-center gap-2 h-9 min-w-0 px-2 rounded-md transition-all duration-150 hover:bg-muted',
    ...
    <div className={cn(
      ...
      <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
        <button
          ...
          className="flex items-center gap-1 rounded-md px-1.5 py-0.5 text-label text-muted-foreground hover:text-foreground hover:bg-muted"
        >
      ...
      <div className="w-72 rounded-xl border border-border/30 bg-popover p-3 shadow-xl animate-in fade-in slide-in-from-bottom-3 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-1 mb-2 flex-wrap">
          <button
            ...
            className={cn('rounded-md px-2 py-0.5 text-label transition-colors', !selectedFolderId ? 'bg-secondary text-secondary-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-muted')}
          >
            All
          </button>
          {folders.map((f) => (
            <button
              ...
              className={cn('rounded-md px-2 py-0.5 text-label transition-colors', selectedFolderId === f.id ? 'bg-secondary text-secondary-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-muted')}
            >
              {f.name}
            </button>
          ))}
          ...
          {loading ? (
            ...
            filteredDocs.map((doc) => (
              <button
                ...
                className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-body hover:bg-muted transition-colors"
              >
