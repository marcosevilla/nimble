...
interface RowPickerState {
  ...
  openPicker: (rowId: string, kind: RowMarkKind) => void
...
export const useRowPickerStore = create<RowPickerState>((set) => ({
  open: null,
  openPicker: (rowId, kind) => set({ open: { rowId, kind } }),
  closePicker: (target) =>
    set((s) => {
      if (!s.open) return {}
...
/** Controlled `open` / `onOpenChange` for one row mark's picker. */
export function useRowPicker(rowId: string, kind: RowMarkKind) {
  const open = useRowPickerStore((s) => s.open?.rowId === rowId && s.open.kind === kind)
  const onOpenChange = (next: boolean) => {
    const store = useRowPickerStore.getState()
    if (next) store.openPicker(rowId, kind)
