...
/// V24 adds reviewed, included label_groups + labels.archived_at; tasks_fts is device-local and excluded.
pub(crate) fn tables_for_version(version: i64) -> Option<Vec<TablePolicy>> {
    if version == 19 { return Some(TABLES.to_vec()); }
    if !(20..=24).contains(&version) { return None; } // schema-v24
