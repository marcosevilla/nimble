...
async fn export_is_repeatable_typed_and_excludes_integration_state() {
    ...
    assert_eq!(format["schema_version"], nimble_core::db::migrations::CURRENT_SCHEMA_VERSION);
...
async fn schema_drift_fails_closed() {
    ...
    let (pool, path) = nimble_core::test_util::file_pool().await;
    sqlx::query("INSERT INTO schema_version(version,description,applied_at) VALUES (?,'future','2026-09-21')")
        .bind(nimble_core::db::migrations::CURRENT_SCHEMA_VERSION + 1)
