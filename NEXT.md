...
- [ ] **Native fix 3 (`9e002d1` + fix round 1)** — compact timer clipping (D) and Up next roving focus (G). Review found C1: Enter on a focused Up next row completed the card task in main view; round 1 committed `fbda5de` (Enter/s guard, Delete Undo + e.repeat, refocus after promote; 269/269). Still needs a scoped re-review (package `9e002d1..fbda5de`), then advance the test worktree to `fbda5de`.
