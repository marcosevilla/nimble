...
- [x] **Merged `codex/focus-checklist-fixes` into local main 2026-09-23** (compact fit root cause = titlebar chrome, card label + expandable description, toasts, sleep 30-min credit). Review Important (wake grace ~100 ms under boundary wakes) fixed `35f2479` (time-based 60 s grace, no boundary wake while a sleep gap is pending); review minor (DarkWake tick used up grace) fixed `753bcbb`; scoped re-review approved. Marco 09-23: chime on wake if a credited sleep crossed a boundary (kept, test-locked); no "Inbox" label. 504 Rust tests. Remaining deferred minors: `../.worktrees/focus-checklist-fixes/.superpowers/sdd/focus-checklist-fixes/review.md` (untracked; keep worktree until copied) (Undo can outlive 10 s on hover; companion toast placement; chrome cache band; expanded-mode clamp; wall-clock stamps inside sleep; tray toast render test). Not pushed, not installed.
- [ ] **Next: Marco re-tests H1, H4, H7, H8 (~10 min)** from `.worktrees/nimble-focus-test` — advance it to main first (`git -C <it> checkout --detach main`, relaunch).
