...
- [ ] **Checklist follow-ups on `codex/focus-checklist-fixes` (2026-09-23, unmerged, awaiting review):** compact companion clipping root-caused and fixed (titlebar is inside the frame Tauri sets), card place label + one-line description, Undo/copy toasts, sleep keeps running with a 30-min cap (chimes on wake if a credited sleep crossed a boundary; no "Inbox" label — Marco 09-23). Wake-grace Important fixed `35f2479`. Evidence: `docs/focus-queue-verification.md` → Checklist follow-ups. Re-run H1, H4, H7, H8, H9 natively after review.
- [ ] **Next: run human checklist H1–H9** (`docs/focus-queue-verification.md`, ~30 min) from the frozen test worktree `.worktrees/nimble-focus-test` — advance it to main first (`git -C <it> checkout --detach main`, relaunch).
