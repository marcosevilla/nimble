...
- [ ] **Finish Google connection in the updated app.** Approved repair `af39e29` (`codex/google-desktop-oauth-fix`) is built, signed and installed in `/Applications/Nimble.app` (2026-09-21). All 330 Rust tests, eight interface tests, desktop/web builds, targeted lint and independent review passed. Native Settings shows the new masked secret field; public client ID remains saved. Next: Google now hides the original secret and offers Add client secret in the existing Nimble Mac client's Information and summary panel. Marco must create/copy a new secret there, paste it into Settings > Phone alerts > Google connection setup, save, and reconnect Google. Browser security rules require user handoff for credential changes; no new secret has been created by the agent. Do not claim live connection until Nimble reports Connected and dedicated calendar exists. Branch remains local, not merged/pushed.
