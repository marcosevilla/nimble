...
- [ ] **Before native checklist item 26:** fix display memo phantom time after pause/resume (ledger Task 9 minor).
- [ ] **Native synthetic run of `docs/focus-native-checklist.md`** on Marco’s Mac (synthetic profile, not the installed app).
- [ ] **PAUSED at Marco’s request after reviewed Task 9. Resume with Task 10** (import) in `codex/focus-absorption` at `/Users/marcosevilla/Developer/marco-task-app/.worktrees/nimble-focus-absorption`. Ledger `.superpowers/sdd/2026-09-22-focus-queue-absorption/progress.md` holds rulings and deferred minors for the final review. Live-data gate: check pre-existing children of local-only tasks for queued Todoist creates before any install. No merge, push, deployment or installation.
