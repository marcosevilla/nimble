...
- [x] **Merged `codex/focus-checklist-fixes` into local main 2026-09-23** (compact fit root cause = titlebar chrome, card label + expandable description, toasts, sleep 30-min credit). Review Important (wake grace ~100 ms under boundary wakes) fixed `35f2479` (time-based 60 s grace, no boundary wake while a sleep gap is pending); review minor (DarkWake tick used up grace) fixed `753bcbb`; scoped re-review approved. Pushed to GitHub 2026-09-23 (`9c9ebbe`). Marco 09-23: chime on wake if a credited sleep crossed a boundary (kept, test-locked); no "Inbox" label. 504 Rust tests. Remaining deferred minors: `../.worktrees/focus-checklist-fixes/.superpowers/sdd/focus-checklist-fixes/review.md` (untracked; keep worktree until copied) (Undo can outlive 10 s on hover; companion toast placement; chrome cache band; expanded-mode clamp; wall-clock stamps inside sleep; tray toast render test). Pushed, not installed.
