...
Updated 2026-09-22 (Focus Queue implementation Tasks 1–9 reviewed; PAUSED before Task 10 at Marco’s request; design facelift loop 1, Stage B merged). Earlier: 2026-09-21 installed Google OAuth repair and verified first live sync. Current status below supersedes earlier installation snapshots.

## Focus Queue absorption — 2026-09-22 (PAUSED after Task 9)
...
- [ ] **Native fix 3 (`9e002d1` + fix round 1)** — compact timer clipping (D) and Up next roving focus (G). Review found C1: Enter on a focused Up next row completed the card task in main view; round 1 (Enter guard, Delete Undo + e.repeat, refocus after promote) was in progress at session clear — check `git status`/log; if uncommitted edits remain in FocusQueueList/FocusQueueTray/FocusView/focusQueueIntents/keyGuard, finish/test/commit them, then scoped re-review (package `9e002d1..HEAD`).
- [ ] Re-test **D** and **G** natively after fix 3; run **L** (sleep) at the Mac. Test app: frozen worktree `.worktrees/nimble-focus-test` (detached; advance with `git checkout --detach <sha>`; Rust changes need a relaunch — see run doc ‘How to re-run’). Synthetic profile `/private/tmp/nimble-backup-test-focus`.
- [ ] Triage the run doc’s “Worth fixing before daily use” list (one batch or fold into final whole-branch review).
- [ ] **PAUSED at Marco’s request after reviewed Task 9 + native run. Resume with native fix 3 wrap-up, then Task 10** (import) in `codex/focus-absorption` at `/Users/marcosevilla/Developer/marco-task-app/.worktrees/nimble-focus-absorption`. Ledger `.superpowers/sdd/2026-09-22-focus-queue-absorption/progress.md` holds rulings and deferred minors for the final review. Live-data gate: check pre-existing children of local-only tasks for queued Todoist creates before any install. No merge, push, deployment or installation.
