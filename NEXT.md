...
- [ ] **Fix `tools/update-app.sh`:** under `set -euo pipefail` the `find` over the missing `apps/desktop/src-tauri/target/...` dir exits 1 and aborts the script silently after bundling (install steps were run by hand). Add `|| true` to that pipeline or drop the dead path.
