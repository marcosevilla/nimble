...
- [ ] **Next: review + merge `codex/focus-checklist-fixes`** (`.worktrees/focus-checklist-fixes`, commits `08f53f7..ae21c3f`: compact fit root cause = titlebar chrome, card label + expandable description, toasts, sleep 30-min credit; 500 Rust / 297 frontend). Review done: **Needs fixes (1 Important)** — wake grace collapses to ~100 ms for timebox/Pomodoro sessions so sleep often isn't credited; fix time-based grace + test, scoped re-review, then merge. Full review + minors: `.worktrees/focus-checklist-fixes/.superpowers/sdd/focus-checklist-fixes/review.md`. Product calls for Marco: chime on wake after a ≤30-min sleep that crossed a boundary; "Inbox" label on local-only tasks. Then Marco re-tests H1, H4, H7, H8 (~10 min) from `.worktrees/nimble-focus-test` advanced to the merged main.
