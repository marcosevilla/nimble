...
- [ ] **Next session: annotate with Agentation** — launch a dev test copy of main (synthetic profile `/private/tmp/nimble-backup-test-*`, `NIMBLE_BACKUP_TEST_ROOT`, frozen worktree), Marco annotates via the dev-only toolbar (`App.tsx` `import.meta.env.DEV`), agent reads via the agentation MCP and turns notes into fixes..
