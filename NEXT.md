...
- [ ] **Marco: review `facelift/agentation-1` in the test copy**, then merge to main + install (not merged, not pushed, not installed). Known: Focus tray footer truncates "From: Today" at the 288px default rail width (tray was built for 340px) — widen the default rail or tighten the footer. The nav trees haven't been checked in the native app (drag regions, window chrome at the top-left).
