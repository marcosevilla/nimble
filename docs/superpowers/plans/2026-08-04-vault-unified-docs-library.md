...
- **⌘K does not search the vault, by decision.** Task 13's "one search across both backends" means `DocsSearch`, the box above the folder tree in the Docs sidebar — not the `CommandBar` palette. ⌘K stays actions-and-navigation only so it doesn't hit a ~1.1k-note FTS index on every keystroke. Marco confirmed this split 2026-08-06 after hitting it in the GUI pass; don't re-litigate. Adding vault hits to ⌘K would need a section header so note results don't bury commands.
