...
- **Open (not fixed):** `t2-row-keys` AC2 Today ("key-opened picker anchors to the row's mark") is flaky — 1–3 of its 4 cases fail per run, at `f4304bd` as well as at this branch's head. The click-opened and key-opened pickers flip to opposite sides of the mark (delta = the picker's height) because the row lands at different scroll positions near the fold. Two test-side fixes (compare relative to the mark; center the row before each open) didn't hold, so it was left for review.
- **Flag for Marco (not changed):** `QuickWins` declares `requires: [Integration::Tasks]`, and `sources.tasks` means "a Todoist token is set". Nothing gates on a manifest's `requires` today, so it's informational — but a future gate would hide Quick wins on a native-only profile. Consider `requires: vec![]` like Due today / Still open.
