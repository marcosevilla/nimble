...
### Enter's default row: "Tasks or create" (Marco, review item 3)

- `defaultIndex(rows, ctx)` in `lib/omnibarRows.ts` takes a `DefaultRowContext { text, type }`, where `type` is the `type:` pill or `'doc'` in `/doc` mode. `selectedIndex` and `freshRow` pass it through, and `Omnibar.tsx` builds it once (`defaultCtx`). The rules, in order:
  1. A non-task type defaults to its group's first row, else its create row (none for `action`).
  2. An empty text defaults to the first task (pill-only query), else the first recent, else -1. So Enter on a bare bar does nothing (review minor 8).
  3. A `note:` / `idea:` / `remember` start defaults to Create note.
  4. A first word in `ACTION_VERBS` defaults to Create task.
  5. Otherwise the first task, else the first create row.
- Notes, docs, vault notes, goals and actions never take the default, so a single action keyword such as "sync" creates (review minor 9).
- `ACTION_VERBS` is the old ⌘K list (`CommandBar.tsx` at `5532805`) **plus "email"**, which the decision names but the old list lacked. `NOTE_PREFIXES` keeps the old `remember` beside `note:` / `idea:`.
- Unit tests in `omnibarRows.test.mjs` cover each rule; the older default-row tests now pass a context.
- e2e:
  - New: `an action verb + Enter creates a task even when a vault note matches` uses "call the dentist", because the mock's `create_document` doesn't add to the searchable docs, so "buy milk" can't be given a doc match. The vault quick-capture note contains "Call the dentist".
  - New: `a matching open task takes Enter once results settle` and `Enter in an empty bar with no recent searches does nothing`.
  - Updated to arrow up to the action or vault row, which no longer takes the default: the vault-note, recent/actions and double-Enter tests.
- Spec §1.7 updated to match.
