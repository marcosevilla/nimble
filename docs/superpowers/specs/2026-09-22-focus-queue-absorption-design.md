...
Amendment 2026-09-23 (Marco's decision after human checklist H8): system sleep no longer pauses a running session. The sleep notice settles a durable checkpoint (the sleep start); the wake notice seen by the same live process credits the sleep, capped at 30 minutes. A sleep of 30 minutes or less keeps running; a longer one credits exactly 30 minutes and pauses at sleep start + 30 min with a readable reason and no sound. Crash, force-quit, ⌘Q, last-surface close, a crash or relaunch across a sleep, and any gap not explained by a sleep/wake pair still recover paused at the last durable checkpoint with no downtime credited. The rows in §2, §4 and §7 below are amended accordingly.
...
| Sleep, wake and crash | **Amended 2026-09-23:** system sleep does not pause; a wake observed by the live process credits the sleep up to 30 minutes, and a longer sleep pauses at sleep start + 30 min. Relaunch stays paused. Persist every 20 seconds while running and at every transition. A heartbeat gap over 40 seconds that no sleep/wake pair explains is an interruption and recovers at the last durable checkpoint. | Counts at most 30 minutes of sleep; never app downtime. A stalled process can undercount; show recovery reason. Never infer hours of work. |
...
| Closing last visible focus surface pauses; restart pauses; sleep keeps running up to a 30-minute credit (amended 2026-09-23); completing a Pomodoro round waits for explicit next action | Avoid hidden/overnight accounting and preserve voluntary Start. Compact/expanded toggles alone are purely visual. |
...
| System sleep (amended 2026-09-23) | Sleep notice: settle a durable checkpoint, keep running, remember the sleep in memory. Wake notice in the same process: credit min(sleep, 30 min); over 30 min pause at sleep start + 30 min, no sound. | Up to 30 min: still running. Longer: paused with "the Mac slept for more than 30 minutes"; Resume is explicit. |
| Suspension gap (no sleep/wake pair) | Use last durable checkpoint; pause work or break. A heartbeat after a sleep notice waits up to 60 s (from the first heartbeat that sees the gap) for the wake notice before applying this rule; commands never wait. | Wake/relaunch requires Resume; recovery explanation. |
