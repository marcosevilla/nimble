...
7. **Enter / default row.** Enter activates the highlighted row. On each query change the highlight goes to the first search result; if there are no results, to the first CREATE row. Arrow keys move across groups. Task-row quick actions ⌥C (complete), ⌥B (AI breakdown), ⌥M (move to project) keep working.
